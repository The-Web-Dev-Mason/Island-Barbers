// --- FAQ TOGGLE FUNCTION (Moved to top for safety) ---
...
async function sendContactForm(event) {
    ...
    if (!name || !phone) {
        showStatus("Missing Info", "Please include your name and phone number.", false);
        return;
    }
    ...
    // Params to match a generic or new EmailJS template
    // Note: You will need to create a new Template in EmailJS with ID 'template_contact'
    // or update this ID to an existing one that accepts these parameters.
    ...
    // Using the same Service ID, but potentially a new Template ID for Contact
    // If you don't have a separate contact template, you might need to create one.
    // For now, using a placeholder ID 'template_contact'.
    const CONTACT_TEMPLATE_ID = 'template_gxzco2j'; // Reusing existing for now, but ideally needs separate one

    // If using the booking template (gxzco2j), it might expect 'barber', 'date', etc.
    // We'll pass empty strings for those to avoid errors if strict.
    const fallbackParams = {
        name: name,
        email: email,
        phone: phone, // If template supports it
        message: message, // If template supports it
        barber: "General Inquiry",
    ...
    emailjs.send(EMAILJS_SERVICE_ID, CONTACT_TEMPLATE_ID, fallbackParams)
        .then(() => {
            showStatus("Message Sent", "Thanks for reaching out! We'll get back to you soon.", true);
            // Clear form
            ...
            showStatus("Error", "Failed to send message. Please call us instead.", false);
        })
        .finally(() => {
            btn.innerText = originalText;
            btn.disabled = false;
